	...
	public void shortestPathGeneration() {
		...
		if (networkType == 1) {
			...
			case 1:
				...
				for (int i = 0; i < tmpPathArray1.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.smallNetwork.nodesCompute[tmpPathArray1[i]]) {
						geneSequence[1] = tmpPathArray1[i];
						VNFnumCount++;
					}
			...
			case 2:
				...
				for (int i = 0; i < tmpPathArray1.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.smallNetwork.nodesCompute[tmpPathArray1[i]]) {
						geneSequence[VNFnumCount + 1] = tmpPathArray1[i];
						VNFnumCount++;
					}
					if (VNFnumCount >= VNFnum) {
						break;
			...
			case 3:
				...
				for (int i = 0; i < tmpPathArray2.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.smallNetwork.nodesCompute[tmpPathArray2[i]]) {
						geneSequence[VNFnumCount + 4] = tmpPathArray2[i];
						VNFnumCount++;
					}
					if (VNFnumCount >= VNFnum) {
						break;
			...
			case 4:
				...
				for (int i = 0; i < tmpPathArray2.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.smallNetwork.nodesCompute[tmpPathArray2[i]]) {
						geneSequence[4] = tmpPathArray2[i];
						VNFnumCount++;
					}
					if (VNFnumCount >= VNFnum) {
						break;
		...
		if (networkType == 2) {
			...
			case 1:
				VNFnum = 1;
				VNFnumCount = 0;
				tmpPathArray1 = Dijkstra.dijkstraPathArray(this.mediumNetwork.linksDelay, geneSequence[0],
						geneSequence[chromLen - 1]);
				for (int i = 0; i < tmpPathArray1.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.mediumNetwork.nodesCompute[tmpPathArray1[i]]) {
						geneSequence[1] = tmpPathArray1[i];
						VNFnumCount++;
					}
					if (VNFnumCount >= VNFnum) {
			...
			case 2:
				...
				for (int i = 0; i < tmpPathArray1.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.mediumNetwork.nodesCompute[tmpPathArray1[i]]) {
						geneSequence[i + 1] = tmpPathArray1[i];
						VNFnumCount++;
					}
					if (VNFnumCount >= VNFnum) {
						break;
			...
			case 3:
				...
				VNFnum = 2;
				VNFnumCount = 0;
				tmpPathArray2 = Dijkstra.dijkstraPathArray(this.mediumNetwork.linksDelay, geneSequence[1],
						geneSequence[chromLen - 2]);
				for (int i = 0; i < tmpPathArray2.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.mediumNetwork.nodesCompute[tmpPathArray2[i]]) {
						geneSequence[i + 4] = tmpPathArray2[i];
						VNFnumCount++;
					}
					if (VNFnumCount >= VNFnum) {
						break;
			...
			case 4:
				...
				VNFnum = 1;
				VNFnumCount = 0;
				tmpPathArray2 = Dijkstra.dijkstraPathArray(this.mediumNetwork.linksDelay, geneSequence[2],
						geneSequence[chromLen - 2]);
				for (int i = 0; i < tmpPathArray2.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.mediumNetwork.nodesCompute[tmpPathArray2[i]]) {
		...
		if (networkType == 3) {
			...
			case 1:
				VNFnum = 1;
				VNFnumCount = 0;
				tmpPathArray1 = Dijkstra.dijkstraPathArray(this.largeNetwork.linksDelay, geneSequence[0],
						geneSequence[chromLen - 1]);
				for (int i = 0; i < tmpPathArray1.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.largeNetwork.nodesCompute[tmpPathArray1[i]]) {
						geneSequence[1] = tmpPathArray1[i];
						VNFnumCount++;
					}
					if (VNFnumCount >= VNFnum) {
			...
			case 3:
				...
				VNFnum = 2;
				VNFnumCount = 0;
				tmpPathArray2 = Dijkstra.dijkstraPathArray(this.largeNetwork.linksDelay, geneSequence[1],
						geneSequence[chromLen - 2]);
				for (int i = 0; i < tmpPathArray2.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.largeNetwork.nodesCompute[tmpPathArray2[i]]) {
						geneSequence[i + 4] = tmpPathArray2[i];
						VNFnumCount++;
					}
					if (VNFnumCount >= VNFnum) {
			...
			case 4:
				...
				VNFnum = 1;
				VNFnumCount = 0;
				tmpPathArray2 = Dijkstra.dijkstraPathArray(this.largeNetwork.linksDelay, geneSequence[2],
						geneSequence[chromLen - 2]);
				for (int i = 0; i < tmpPathArray2.length; i++) {
					int nodeCost = SfcUtil.nodeComsuption(SfcUtil.smallInstanceNodeComsuption);
					if (VNFnumCount < VNFnum && nodeCost < this.largeNetwork.nodesCompute[tmpPathArray2[i]]) {
